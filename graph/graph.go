...
// Graph holds a graph data structure
type Graph struct {
	lock  sync.RWMutex
...
func (g *Graph) NewNode(key string, value interface{}) error {
	g.lock.Lock()
	defer g.lock.Unlock()
...
func (g *Graph) Value(key string) (interface{}, error) {
	g.lock.RLock()
...
func (g *Graph) UpdateValue(key string, value interface{}) error {
	g.lock.Lock()
	defer g.lock.Unlock()
...
func (g *Graph) NewEdge(from, to string) error {
	g.lock.Lock()
	defer g.lock.Unlock()
...
func (g *Graph) Edges(from string) ([]string, error) {
	var edges []string
