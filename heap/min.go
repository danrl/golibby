...
// MinHeap represents a max heap instance
type MinHeap struct {
	lock sync.RWMutex
	data []int
}
...
// Pop retrieves the smallest item from the heap and rebalances the heap if
// necessary
func (mh *MinHeap) Pop() (int, error) {
	mh.lock.Lock()
	defer mh.lock.Unlock()
	if len(mh.data) == 0 {
		return 0, ErrorNoData
	...
	for i := 0; i < len(mh.data); {
		...
		if li < len(mh.data) && mh.data[li] < mh.data[i] {
			if ri < len(mh.data) && mh.data[ri] < mh.data[li] {
				// right child is even smaller than left child, swap right child
				mh.data[ri], mh.data[i] = mh.data[i], mh.data[ri]
				i = ri
			} else {
				// left child is smaller than new node, swap left child
				mh.data[li], mh.data[i] = mh.data[i], mh.data[li]
				i = li
			}
		} else if ri < len(mh.data) && mh.data[ri] < mh.data[i] {
			// right child is smaller than new new, swap right child
			mh.data[ri], mh.data[i] = mh.data[i], mh.data[ri]
			i = ri
		} else {
